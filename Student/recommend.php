...
function getRecommendation($matrix,$person)
{
	...
	foreach($matrix as $otherPerson=>$value)
	{
		if($otherPerson != $person)
		{
			...
			foreach($matrix[$otherPerson] as $key=>$value) //checking if one of the similar user of have rated another teacher 
			{
				if(!array_key_exists($key,$matrix[$person]))
				{
					...
					$total[$key] += $matrix[$otherPerson] [$key]*$sim; //numerator
